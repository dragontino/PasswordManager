...
class DataRepository(
    ...
    private sealed class Reference(
        val name: String,
        val kotlinClass: KClass<out UsersData>
    ) {
        companion object {
            val valuesMap: Map<DataType, Reference> = mapOf(
                DataType.Website to Websites,
                DataType.Bank to Banks
            )
        }

        object Websites : Reference(
            name = "Websites",
            kotlinClass = Website::class
        )

        object Banks : Reference(
            name = "Banks",
     ...
     * @param dataUpdates словарь, ключами которого являются имена полей класса, а значениями – новые значения этих полей
    ...
    suspend fun <T : Any?> updateData(
        ...
        val ref = when (dataType) {
            DataType.Bank -> Reference.Banks.name
    ...
    /**
     * Функция, которая возвращает список всех данных типа [dataType] в отсортированном виде.
     * В качестве ключа сортировки используется [UsersData] класс.
     *
     * В отличие от [getDataList], эта функция автоматически возвращает новые данные,
     ...
     */
    fun fetchDataList(dataType: DataType = DataType.All): Flow<Map<String, UsersData>> =
        Reference.valuesMap[dataType]
            ?.let { reference ->
                fetchDataMap(
                    tableName = reference.name,
                    kClass = reference.kotlinClass
                ).map { map ->
            ...
            ?: Reference
                .valuesMap
                .map { fetchDataMap(tableName = it.value.name, kClass = it.value.kotlinClass) }
                .combine { map, map2 ->
                    map + map2
                }
                .map { map ->
    ...
    suspend fun getDataList(
        ...
        getDatabaseReference(uid)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val dataTables = Reference
                        .valuesMap
                        .filter { it.key == dataType }
                        .ifEmpty { Reference.valuesMap }
                        .values
                    ...
                    val resultList = dataTables
                        .map { pair ->
                            snapshot
                                ...
                                .children
                                .filter { it.key != null && it.value != null }
                                .associate { it.key!! to it.getValue(pair.kotlinClass.java)!! }
                                .onEach { entry ->
                                    entry.value.decrypt { it.decrypt(uid) }
                                }
                        }
    ...
    suspend fun getData(
        ...
        val ref = Reference.valuesMap[dataType]!!
    ...
    suspend fun deleteData(
        ...
        val dataTable = Reference.valuesMap[dataType]?.name!!
